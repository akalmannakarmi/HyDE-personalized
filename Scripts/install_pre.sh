...
if pkg_installed grub && [ -f /boot/grub/grub.cfg ]; then
    ...
    if [ ! -f /etc/default/grub.hyde.bkp ] && [ ! -f /boot/grub/grub.hyde.bkp ]; then
        ...
        print_log -g "[bootloader] " -b "set :: " "grub theme // Retroboot"
        echo ""
        # shellcheck disable=SC2154
        [ "${flg_DryRun}" -eq 1 ] || sudo tar -xzf "${cloneDir}/Source/arcs/Grub_Retroboot.tar.gz" -C /usr/share/grub/themes/
...
if [ -f /etc/pacman.conf ] && [ ! -f /etc/pacman.conf.hyde.bkp ]; then
    ...
    [ "${flg_DryRun}" -eq 1 ] || sudo pacman -Syyu ${use_default:+"$use_default"}
    [ "${flg_DryRun}" -eq 1 ] || sudo pacman -Fy ${use_default:+"$use_default"}
else
